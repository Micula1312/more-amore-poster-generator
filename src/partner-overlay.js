...
if (stage && input1 && input2) {
  ...
  Object.assign(overlay.style, {
    ...
    justifyContent: 'flex-end', gap: '14px', zIndex: '50'
  });
  ...
  const slots = [0, 1].map(() => {
    ...
    Object.assign(img.style, {
      display: 'none', width: 'auto', height: 'auto', maxWidth: '15%', maxHeight: '8%',
      objectFit: 'contain', flex: '0 0 auto'
    });
    overlay.appendChild(img);
    return img;
  });

  // White versions are generated from the original PNG alpha channel.
  // This preserves transparency and turns every visible logo pixel pure white.
  ...
  function makeWhiteAlphaLogo(sourceImage, done) {
    ...
    white.onload = () => done(white, c.toDataURL('image/png'));
    white.src = c.toDataURL('image/png');
  }
  ...
  setTimeout(() => {
    ...
    exportButton.onclick = () => {
      ...
      const maxW = 150, maxH = 84, rightMargin = 72, bottomMargin = 72, gap = 24;
